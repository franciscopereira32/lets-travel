...
exports.signUpPost = [
    //Validate data
    check('first_nama').isLength({ min: 1 }).withMessage('First name must be specified')
    ...
    .withMessage('Invalit password, password much be a minimum of 6 caracteres'),

    check('confirm_password')
    .custom(( value, { req } ) => value == req.body.password)
    .withMessage('Password addresses do not match'),
    ...
    (req, res, next) => {
        ...
        if(!errors.isEmpty()) {
            //There are errors
            res.render('sigin_up', {title: 'Please fix the following errors:'});
